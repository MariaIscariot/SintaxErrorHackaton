...
const ClickBait = () => {
  ...
  const fetchClickbaitScore = async () => {
    ...
    try {
      const response = await axios.post(
        "https://0411-93-113-114-106.ngrok-free.app/fake_check",
        {
          news_url:
  ...
  const getBarColor = (score: number | null) => {
    ...
    return "#F44336"; // Red for high score
  };
  ...
  return (
    <div className="bg-white rounded-xl w-full h-full p-4">
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-semibold">Fake Check</h1>
      </div>
      ...
      {loading ? (
        ...
        <div className="relative w-full h-[75%]">
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center">
            ...
            <p className="text-sm text-gray-400">Fake check score</p>
          </div>
        </div>
      ) : (
